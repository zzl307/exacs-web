...
class UserController extends Controller
{
	...
	public function addUser()
	{
		if (request()->isMethod('POST'))
		{
			...
			Mail::raw($content, function ($message) use ($user) {
				...
				$message->subject('exands设备管理系统账号');
	...
	public function resetUserPassword()
	{
		...
		try {
			Mail::raw($plaintxt, function ($message) use ($user) {
				$message->from('noreply@exands.cn', '');
				$message->subject('exands设备管理系统密码更新');
				$message->to($user->email);
			});
		} catch (\Exception $e) {
	...
	public function getRolePermissions()
	{
		...
		$permissions = array();
		foreach (AdminPermission::orderBy('keyword', 'asc')->get() as $p)
		{
			$permission['id'] = $p->id;
	...
	{
		$permissions = AdminPermission::orderBy('keyword', 'asc')->get();
